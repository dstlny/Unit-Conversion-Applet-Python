...
class AppFrame(wx.Frame):
    ...
    def onInitialCurrencyLoad(self):
        self.filePath = path.dirname(path.realpath(__file__))
...
class algorithmGeneration:
    ...
    class md5:
        ...
        def produceFileHash(self, byte):
            self.hashString = md5(str([b for b in byte]).encode("utf-8")).hexdigest()

        def produceDirHash(self, byte):
            self.hashString = md5(str([b for b in byte]).encode("utf-8")).hexdigest()
            
        def produceDirMetaHash(self, fileSize):
            self.hashString = md5(str(fileSize).encode("utf-8")).hexdigest()
    ...
    class sha256:
        ...
        def produceFileHash(self, byte):
            self.hashString = sha256(str([b for b in byte]).encode("utf-8")).hexdigest()

        def produceDirHash(self, byte):
            self.hashString = sha256(str([b for b in byte]).encode("utf-8")).hexdigest()

        def produceDirMetaHash(self, fileSize):
            self.hashString = sha256(str(fileSize).encode("utf-8")).hexdigest()
    ...
    class sha3_512:
        ...
        def produceFileHash(self, byte):
            self.hashString = sha3_512(str([b for b in byte]).encode("utf-8")).hexdigest()

        def produceDirHash(self, byte):
            self.hashString = sha3_512(str([b for b in byte]).encode("utf-8")).hexdigest()

        def produceDirMetaHash(self, fileSize):
            self.hashString = sha3_512(str(fileSize).encode("utf-8")).hexdigest()
